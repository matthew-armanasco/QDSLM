...
Public Class Form1
    ...
    Private Sub PortStatusCallback(<MarshalAs(UnmanagedType.LPStr)> ByVal portname As String, ByVal status As NKTPDLL.PortStatusTypes, ByVal curScanAdr As Byte, ByVal maxScanAdr As Byte, ByVal foundType As Short)
        Dim statusMsg As String = vbCrLf & "PortInfo Callback: " & portname & " status:" & status.ToString() & " curScanAdr:" & curScanAdr.ToString() & " maxScanAdr:" & maxScanAdr.ToString() & " foundType:" & foundType.ToString("X4")
    ...
    Private Sub DeviceStatusCallback(<MarshalAs(UnmanagedType.LPStr)> ByVal portname As String, ByVal devId As Byte, ByVal status As NKTPDLL.DeviceStatusTypes, ByVal devDataLen As Byte, <MarshalAs(UnmanagedType.LPArray, SizeParamIndex:=3)> ByVal devData As Byte())
        ...
        Select Case status
            ...
            Case NKTPDLL.DeviceStatusTypes.DeviceTypeChanged
                ' 2 - devData contains 1 unsigned short with DeviceType (module type).
                statusMsg = statusMsg & "type: 0x" & devData(0).ToString("X4")
    ...
    Private Sub RegisterStatusCallback(<MarshalAs(UnmanagedType.LPStr)> ByVal portname As String, ByVal devId As Byte, ByVal regId As Byte, ByVal status As NKTPDLL.RegisterStatusTypes, ByVal regType As NKTPDLL.RegisterDataTypes, ByVal regDataLen As Byte, <MarshalAs(UnmanagedType.LPArray, SizeParamIndex:=5)> ByVal regData As Byte())
        Dim statusMsg As String = vbCrLf & "DeviceInfo Callback: " & portname & " devId: " & devId.ToString() & " regId:" & regId.ToString("X2") & " status:" & status.ToString() & " regType: " & regType.ToString() & " "
