...
namespace DLL_Example_CS_Callback
{
    public partial class Form1 : Form
    {
        ...
        private static void PortStatusCallback([MarshalAs(UnmanagedType.LPStr)]String portname, NKTPDLL.PortStatusTypes status, byte curScanAdr, byte maxScanAdr, short foundType)
        {
            string statusMsg = "\r\nPortInfo Callback: " + portname + " status:" + status.ToString() + " curScanAdr:" + curScanAdr.ToString() + " maxScanAdr:" + maxScanAdr.ToString() + " foundType:" + foundType.ToString("X4");
        ...
        private static void DeviceStatusCallback([MarshalAs(UnmanagedType.LPStr)]String portname, byte devId, NKTPDLL.DeviceStatusTypes status, byte devDataLen, [MarshalAs(UnmanagedType.LPArray, SizeParamIndex=3)]byte[] devData)
        {
            ...
            switch ((NKTPDLL.DeviceStatusTypes)status)
            {
                ...
                case NKTPDLL.DeviceStatusTypes.DeviceTypeChanged:
                    ...
                    statusMsg += "type: 0x" + devData[0].ToString("X4");
                    break;
