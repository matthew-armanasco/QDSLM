...
enum tDeviceModeTypes
{
    ...
    DevModeError = 5,           //!< 5 - The device is in an error state. \deprecated No longer used.
...
enum tDeviceStatusTypes
{
    ...
    DeviceTypeChanged = 2,          //!<  2 - devData contains 1 unsigned short with DeviceType (module type). \note Was originally a byte, but since we now has moduletypes above 0xFF, this has been changed to a short(unsigned 16bit).
    DevicePartNumberChanged = 3,    //!<  3 - devData contains a zero terminated string with partnumber. \deprecated No longer available.
 ...
 * \param hostPort Pointer to a preallocated unsigned short where the function will store the local port number.
 ...
 * \param clientPort Pointer to a preallocated unsigned short where the function will store the client port number.
 ...
 *                       \arg 1 the busscanning is set to legacy mode and fixes the masterId at address 66(0x42). Some older/legacy modules does not accept masterIds other than 66(0x42).
 * \sa ::getLegacyBusScanning, ::setSpecificBusScanningRange and ::getSpecificBusScanningRange
 * \note The buscanning can be defined with ::setLegacyBusScanning and ::setSpecificBusScanningRange according to following settings:
...
/*!
 * \brief Sets specific busscanning address range, on/off.
 * \param specificScanning \arg 0 the specific busscanning range is set to Off (Normal mode).
 *                         \arg 1 the specific busscanning range is set to On. And the range is set to the parameters startAddress and endAddress.
 * \param startAddress The specific start address.
 * \param endAddress The specific end address.
 * \sa ::getSpecificBusScanningRange, ::setLegacyBusScanning and ::getLegacyBusScanning
 */
...
/*!
 * \brief Gets specific busscanning address range and status.
 * \param specificScanning Pointer to a char where the function will store the current status. 0 = Off, 1 = On
 * \param startAddress Pointer to an unsigned char where the function will store the current startAddress.
 * \param endAddress Pointer to an unsigned char where the function will store the current endAddress.
 * \sa ::setSpecificBusScanningRange, ::setLegacyBusScanning and ::getLegacyBusScanning
 */
 ...
 * \param devDataLen Number of bytes in devData.
 * \param devData The device data as specified in ::tDeviceStatusTypes status.
 * \note Please note that due to risk of circular runaway leading to stack overflow, it is not allowed to call functions in the DLL from within the callback function.
 * If a call is made to a function in the DLL the function will therefore return an application busy error.
 */
 ...
 * \param portname Zero terminated string giving the current portname.
 * \param devId The Id/Address of the device.
 * \param regId The register number.
...
typedef struct lvPortStatusStruct
{
    ...
    unsigned short foundType; //!< When status is ::PortScanDeviceFound this value will represent the found module type. \note Was originally a byte, but since we now has moduletypes above 0xFF, this has been changed to a short.
