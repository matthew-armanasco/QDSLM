...
# NOTE you need to remove the libiomp5md.dll out on the C:\\Program Files\\Point Grey Research\\FlyCapture2\\bin64. It confuses python a lot and makes the kernal crash due to multiple initilisation of the libiomp5md.dll.
# NOTE some time python packages will get a bit messed up and libiomp5md.dll will be initialised several times. This will cause the kernal to crash and if you look at the kernal log you will see 
# it is becuase of the libiomp5md.dll being initialised multiple times you can genrally fix with a conda update --all
# If all things fail and you cant get rid of the kernal crash due to multiple initialisations of libiomp5md.dll 
# import os
...
class FLIRCameraObject():
    def __init__(self,CalibrationFile=None,PixelSize=6.9e-6):
        ...
        # get the gain exposure property 
        ...
        i=0
        print(error)
        print(rawImage.cols,rawImage.rows)
        print(rawImage.receivedDataSize)
        while (not (error==0 and rawImage.receivedDataSize==rawImage.cols*rawImage.rows)):# just keep trying to grab a frame until you get one
            print(i)
            error = fc2lib.fc2RetrieveBuffer(context, ctypes.byref(rawImage));
            print(rawImage.receivedDataSize)
            i=i+1
        ...
        print(rawImage.receivedDataSize)
        print(hex(rawImage.format.value))
        Framedtype=np.uint8
        ...
        self.CamObject=CamForm.GeneralCameraObject("FLIRCamera",CalibrationFile,self.Nx,self.Ny,FrameWidth,FrameHeight,FrameDim,Framedtype,FrameBuffer,PixelSize,Exposure,0,Gain,'')
...
def FLIRFrameCaptureThread(queue,Cam_Calibtation,SetCalibrationEvent,
                           GetFrameFlag,GetFrameFlag_digholo,
                           terminateCamFlag,FrameObtained,
    ...
    Frame_int=np.zeros((FrameHeight,FrameWidth),dtype=np.uint8)
    ...
    while not terminateCamFlag.is_set():
        if (ContinuesMode.is_set()):
            error = fc2lib.fc2RetrieveBuffer(context, ctypes.byref(rawImage));
            while (not (error==0 and rawImage.receivedDataSize==FrameHeight*FrameWidth)):# just keep trying to grab a frame until you get one
                error = fc2lib.fc2RetrieveBuffer(context, ctypes.byref(rawImage));
            frameBufferPtr=rawImage.pData
            frame = np.ctypeslib.as_array(frameBufferPtr,shape=(FrameHeight,FrameWidth))
            # frame = np.ctypeslib.as_array(frameBufferPtr,shape=(FrameWidth,FrameHeight))
        ...
        if(SetExposureFlag.is_set()):
            ...
            if ( (Exposure)>=0.005125999450683594 and (Exposure)<61.5157470703125):#milliseconds
                ...
                Properties.onePush=0
                Properties.absControl=1
                Properties.absValue=Exposure
                error=fc2lib.fc2SetProperty(context,ctypes.byref(Properties) );
                shared_flag_int.value=1
                fc2lib.fc2GetProperty(context,ctypes.byref(Properties) );
                shared_float.value=Properties.absValue
...
#         error_code = fc2lib.fc2CreateContext(ctypes.byref(self.context))
#         #see if there are any Cameras 
#         numCameras=ctypes.c_uint()
#         error = fc2lib.fc2GetNumOfCameras((self.context), ctypes.byref(numCameras));
#         #Get a ID for the camera 
#         index=ctypes.c_uint(0)
#         error = fc2lib.fc2GetCameraFromIndex(self.context, index, ctypes.byref(fc2Guid));
#         # connect to the camera 
#         error = fc2lib.fc2Connect(self.context, ctypes.byref(fc2Guid));
#         #Start a capature
#         error = fc2lib.fc2StartCapture(self.context);
...
#         # Need to make this c structure to which the image and some of the other information will be in
#         self.rawImage = fc2Image() #fc2Image rawImage;
#         # Call fc2CreateImage, passing a reference to rawImage this initallises all the c variables in rawImage structure
#         error = fc2lib.fc2CreateImage(ctypes.byref(self.rawImage))
#         # Now we can actually get a frame some times you will ask for a fame too quickly so you need to keep asking until you get one
#         error = fc2lib.fc2RetrieveBuffer(self.context, ctypes.byref(self.rawImage));
#         i=0
#         print(error)
#         print(self.rawImage.cols,self.rawImage.rows)
#         print(self.rawImage.receivedDataSize)
#         while (not (error==0 and self.rawImage.receivedDataSize==self.rawImage.cols*self.rawImage.rows)):# just keep trying to grab a frame until you get one
#             print(i)
#             error = fc2lib.fc2RetrieveBuffer(self.context, ctypes.byref(self.rawImage));
#             print(self.rawImage.receivedDataSize)
#             i=i+1
#         # convert the c stuff to python stuff essentially taking the pointer and making it into a np array    
#         frameBufferPtr=self.rawImage.pData
#         print(frameBufferPtr)
#         self.Ny = self.rawImage.cols
#         self.Nx = self.rawImage.rows
#         #  unsigned int rows;
#         # unsigned int cols;
#         # unsigned int stride;
#         # unsigned char* pData;
...
#         # fc2ImageImpl imageImpl;
#         print(self.rawImage.receivedDataSize)
#         print(hex(self.rawImage.format.value))
#         Framedtype=np.uint8
#         frame = np.ctypeslib.as_array(frameBufferPtr,shape=(self.Nx,self.Ny))
#         # self.frame=copy.deepcopy(frame1)
#         # FrameDim=self.frame.shape
#         # print("framedims")
#         # print(FrameDim)
#         # print(self.frame.dtype)
#         # print(self.frame[100,100])
#         # FrameHeight = int(FrameDim[0])
#         # FrameWidth = int(FrameDim[1])
#         # # print(FrameDim)
#         plt.figure(201)
...
#     def __del__(self):
#         error = fc2lib.fc2DestroyImage(ctypes.byref(self.rawImage));
#         # Need to stop capture and destory the connection to the context
#         error = fc2lib.fc2StopCapture(self.context);
#         fc2lib.fc2DestroyContext(self.context)
#     #     """Destructor to disconnect from camera."""
#     #     print(self.CamObject.CameraType +" Class has been destroyed")
#     #     self.CamObject.terminateCamera.set()# stop the camera thread
#     #     self.CamObject.shm.close() # close access to shared memory
